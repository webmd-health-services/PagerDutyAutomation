...
{
    # Don't do anything since Initialize-PagerDutyAutomation.ps1 imports the module.
}
...
Describe ('PagerDutyAutomation.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_PagerDutyAutomation' -Exists
    }
}

Describe ('PagerDutyAutomation.command verbs') {
...
Describe ('PagerDutyAutomation.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'PagerDutyAutomation' -CommandType Function,Cmdlet,Filter))
        {
